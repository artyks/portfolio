...
const loadAssetsManagerConfig = (): AssetsManagerConfig => {
  ...
  if (!process.env.GOOGLE_STORAGE_BUCKET_NAME) {
    throw new Error("Provide 'GOOGLE_STORAGE_BUCKET_NAME' env variable");
  }
  if (!process.env.GOOGLE_STORAGE_CDN_URL) {
    throw new Error("Provide 'GOOGLE_STORAGE_CDN_URL' env variable");
  }
  ...
  return {
    ...
    GOOGLE_STORAGE_CDN_URL: process.env.GOOGLE_STORAGE_CDN_URL,
